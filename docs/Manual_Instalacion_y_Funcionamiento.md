...
Sistema para diseñar y administrar los patrones de tejido de los telares de la planta.
Tiene dos partes que ya están conectadas entre sí:

- **Frontend**: la página donde se diseñan los patrones (cuadrícula de pasadas, colores de hilo)
  y se simula el tejido. Corre en el navegador.
- **Backend**: una API (Node.js + Express) conectada a una base de datos PostgreSQL, que
  guarda los patrones, registra qué telar está tejiendo qué, y mantiene historial de producción.

El mismo servidor Express sirve tanto la API como la página web: al abrir
`http://localhost:3000` el frontend ya está conectado al backend y guarda
todo en la base de datos (ya no depende de `localStorage`).

A futuro, este sistema va a ser el puente entre la app (web o móvil) y los
telares reales con microcontroladores ESP32.
...
```
Proyecto_Control_Insercion_Trama/
├── backend/
│   ├── src/
...
│   │   ├── utils/                 Helpers (validación)
│   │   ├── middleware/            Manejo de errores
│   │   ├── controllers/           Lógica de negocio
│   │   └── routes/                Rutas de la API
│   ├── public/
│   │   ├── index.html             ← la página, ya conectada al backend
│   │   └── styles.css
│   ├── Dockerfile
│   ├── docker-compose.yml
...
└── docs/
    ...
    └── Analisis_Frontend_y_Plan_Backend.md       análisis técnico completo
```
...
4. Sirve la página en **http://localhost:3000**
...
Abrí **http://localhost:3000** en el navegador. La página ya está conectada a la base de datos.
...
Abrila en el navegador — debería cargar la página completa. Para confirmar que el backend
...
Si eso responde, el servidor y la base de datos están bien. Abrir
**http://localhost:3000** en el navegador y la página debe cargar completa.

---

## 6. Cómo funciona la página

### Pantalla de Inicio
Portada del sistema. Muestra el último patrón trabajado (nombre, dimensiones,
fecha de modificación) y accesos rápidos a crear uno nuevo o ir a la biblioteca.
La información viene de la base de datos al cargar la página.

### Pantalla Editor
Donde se diseña un patrón de tejido. **Cada acción se guarda solo en la base
de datos**, no hace falta apretar "Guardar" para que algo quede registrado:

- **Nombre del patrón**: el nombre con el que se va a guardar en la base de datos.
  Si ya existe un patrón con ese nombre, el sistema lo avisa y no deja guardarlo.
- **Filas y columnas**: definen el tamaño de la cuadrícula (filas = hilos de urdimbre,
  columnas = pasadas de trama). Cambiar el tamaño se guarda solo.
- **Cuadrícula**: tocando una celda se le asigna un número de pasadas. Las celdas
  con pasadas > 0 se muestran en color. Cada toque se guarda solo, al instante.
- **Color por fila**: a cada fila se le puede asignar un color de hilo para ver
  cómo va a quedar la tela. También se guarda solo.
- **Botón Guardar**: abre un diálogo con dos opciones (sigue existiendo para
  ponerle nombre definitivo o para arrancar un dibujo nuevo):
  - *Guardar y crear uno nuevo*: guarda en la base y limpia el editor para empezar otro.
  - *Guardar y Reemplazar*: guarda y se queda en el mismo patrón para seguir editando.
- **Controles de simulación** (▶ ⏸ ⏹ ↺): animan visualmente la grilla **y al
  mismo tiempo avisan al telar real** (por ahora hay un solo telar, se usa
  automáticamente — no hay que elegir nada). ▶ asigna el patrón al telar y
  empieza a avanzar; ↺ retrocede usando la posición real que devuelve el
  servidor; ⏹ cierra esa producción en el historial. La conexión con el
  hardware físico (ESP32) es la única parte que todavía no existe — hasta
  entonces, cada "pasada" se registra en la base de datos pero no mueve nada
  físico todavía.
- **Exportar ficha técnica**: descarga un **PDF real** con el nombre, dimensiones,
  fecha y la grilla completa coloreada igual que en el editor (cada celda muestra
  el número de pasadas, con el color de hilo de su fila). Se genera en el navegador
  con la librería `jsPDF`, que está incluida localmente en el proyecto — no depende
  de internet ni de un servicio externo, así que funciona aunque la planta no tenga
  conexión.

### Pantalla Biblioteca
Lista todos los patrones guardados en la base de datos, con miniatura de cada uno.

- **Buscar**: filtra por nombre en tiempo real (búsqueda local dentro de los ya cargados).
- **Cargar**: abre el patrón en el editor para editarlo o simularlo.
- **Eliminar**: borra el patrón de la base de datos (pide confirmación). Si el patrón tiene
  historial de producción asociado, no lo deja borrar (para no perder el historial).
...
|---|---|
| Diseñar patrones en el editor | ✅ Funcionando |
| Guardar / editar / borrar patrones en la base de datos | ✅ Funcionando |
| Autoguardado de cada acción (celda, color, tamaño) sin apretar "Guardar" | ✅ Funcionando |
| Cargar patrones desde la base de datos al iniciar | ✅ Funcionando |
| Biblioteca de patrones con búsqueda | ✅ Funcionando |
| Errores del frontend registrados en la base de datos | ✅ Funcionando |
| Historial de producción (tablas de BD) | ✅ Funcionando (estructura lista; sin pantalla visual todavía) |
| Telar único automático (sin selector, por ahora hay uno solo) | ✅ Funcionando |
| Play/Pausa/Detener/Retroceder conectados al telar (posición real en la base) | ✅ Funcionando |
| PDF real de ficha técnica | ✅ Funcionando (se genera en el navegador, sin depender de internet) |
| Selector visual de telar/máquina (para cuando haya más de uno) | ⏳ Pendiente |
| Pantalla visual de historial de producción | ⏳ Pendiente |
| Conexión al telar real (ESP32) | ⏳ Próxima etapa |
| Versión de aplicación móvil | ⏳ A futuro |
...
| La página carga pero la biblioteca aparece vacía | Es normal si es la primera vez. Crear un patrón desde el editor. |
...
| La página no carga en el navegador | Verificar que el servidor esté corriendo y que `http://localhost:3000/api/health` responda. |
| No se ve la tipografía Century Gothic | Es una fuente comercial, no se puede empaquetar. Si el sistema operativo no la tiene instalada, la página cae automáticamente a `Questrial` (Google Fonts), que es la alternativa visual más parecida. |
